...
if not _is_truthy_env('ENABLE_LEGACY_FLASK'):
    raise RuntimeError(
        "Legacy Flask service is disabled. "
        "Set ENABLE_LEGACY_FLASK=1 for temporary internal migration use."
    )
...
app.config.update(
    ...
    MAX_CONTENT_LENGTH=_int_env('LEGACY_MAX_CONTENT_LENGTH', 2 * 1024 * 1024),
)
LEGACY_HTTP_TIMEOUT_SECONDS = _int_env('LEGACY_HTTP_TIMEOUT_SECONDS', 8)
legacy_cors_origins = _parse_legacy_cors_origins()
if legacy_cors_origins:
    supports_credentials = _is_truthy_env('LEGACY_CORS_SUPPORTS_CREDENTIALS')
    if supports_credentials and '*' in legacy_cors_origins:
        raise RuntimeError(
            "Invalid LEGACY_CORS_ORIGINS configuration: wildcard '*' cannot be used when "
            "LEGACY_CORS_SUPPORTS_CREDENTIALS=true."
        )
    CORS(
        app,
        resources={r"/*": {"origins": legacy_cors_origins}},
...
RATE_LIMIT_DB = os.path.join(BASE_DIR, 'rate_limits.sqlite')
...
def init_rate_limit_db():
    with get_rate_limit_conn() as conn:
        conn.execute(
            '''
        ...
        conn.execute(
            ...
            ON rate_limit_hits(bucket, key, hit_at)
            '''
        )
...
def is_rate_limited(bucket, key, limit, window_seconds):
    ...
    retention_cutoff = now_ts - max(window_seconds, 24 * 60 * 60)
    ...
    with get_rate_limit_conn() as conn:
        ...
        hit_count = conn.execute(
            ...
            WHERE bucket = ? AND key = ? AND hit_at >= ?
            ''',
            (bucket, key, cutoff),
        ).fetchone()[0]
...
<body><h3>CSRF 校验失败，请刷新页面后重试。</h3></body>
</html>
                '''
            ), 403
...
def verify_user_password(user_data, password):
    password_hash = user_data.get("password_hash")
    if password_hash:
        return check_password_hash(password_hash, password)
    return user_data.get("password") == password

def set_user_password(user_data, password):
...
def login():
    ...
    if request.method == 'POST':
        ...
        rate_limit_key = f"{get_client_ip()}:{username.strip().lower()}"

        if is_rate_limited('login', rate_limit_key, 10, 600):
...
<body>
    <div class="login-container">
        ...
        if username in users and verify_user_password(users[username], password):
...
def register():
    ...
    if request.method == 'POST':
        ...
        if is_rate_limited('register', get_client_ip(), 5, 3600):
            ...
            if username in users:
                ...
                set_user_password(users[username], password)
...
@csrf_protect
def logout():
    if is_rate_limited('logout', session.get('username', get_client_ip()), 20, 300):
...
@csrf_protect
def delete_account():
    if is_rate_limited('delete_account', session.get('username', get_client_ip()), 3, 3600):
        return jsonify({"error": "操作过于频繁，请稍后再试"}), 429
...
@csrf_protect
def change_password():
    if is_rate_limited('change_password', session.get('username', get_client_ip()), 10, 600):
        return jsonify({"error": "修改过于频繁，请稍后再试"}), 429
    ...
    current_config = load_config()
    if username in current_config['users']:
        set_user_password(current_config['users'][username], new_password)
        save_config(current_config)
...
@csrf_protect
def upload_file():
    if is_rate_limited('upload', session.get('username', get_client_ip()), 20, 300):
...
def home(token, bin_param, key):
   ...
   'Content-Type': 'application/octet-stream'
    ...
    response = requests.request(
        ...
        headers=headers,
        data=payload,
        timeout=LEGACY_HTTP_TIMEOUT_SECONDS,
    )
...
if __name__ == '__main__':
    allow_legacy = os.environ.get('ENABLE_LEGACY_FLASK', '').lower() in ('1', 'true', 'yes')
    if not allow_legacy:
        raise SystemExit(
            "Legacy Flask service is disabled by default. "
            "Set ENABLE_LEGACY_FLASK=1 for temporary internal migration use."
        )

    host = os.environ.get('FLASK_RUN_HOST', '127.0.0.1')
    port = int(os.environ.get('FLASK_RUN_PORT', '5000'))
    allow_public_bind = os.environ.get('ALLOW_LEGACY_FLASK_PUBLIC_BIND', '').lower() in ('1', 'true', 'yes')
