    ...
    const taskPromises = selectedTokens.value.map(async (tokenId) => {
      ...
      try {
        ...
        const maxRefreshAttempts = Math.max(
          1,
          Number(
            batchSettings.smartDepartureMaxRefreshAttempts
            || batchSettings.smartDepartureMaxRefreshTimes
            || 30,
          ) || 30,
        );
        ...
        try {
          ...
          currentRoleId = roleRes?.role?.roleId
            ? String(roleRes.role.roleId)
            : null;
          addLog({
            time: new Date().toLocaleTimeString(),
        ...
        try {
          ...
          sortedHelpers = Object.values(membersMap)
            ...
            .map((m) => ({
              ...
              lineupText: extractMemberLineupText(m),
            }))
            .map((m) => {
              const lineupLower = String(m.lineupText || "").toLowerCase();
          ...
          if (lineupAnalysisEnabled && preferredLineupKeywords.length > 0) {
            const matchedCount = sortedHelpers.filter(
              (m) => m.lineupPriority !== EMPTY_PRIORITY,
            ).length;
            addLog({
              time: new Date().toLocaleTimeString(),
              message: `${token.name} 护卫阵容分析：关键词[${preferredLineupKeywords.join("、")}]，命中${matchedCount}/${sortedHelpers.length}`,
              type: "info",
            });
          }
        ...
        const assignHelperIfNeeded = async (car) => {
          ...
          if (bestHelper) {
            ...
            addLog({
              time: new Date().toLocaleTimeString(),
              message: `${token.name} 车辆[${gradeLabel(car.color)}]自动分配护卫: ${bestHelper.name}${bestHelper.lineupText ? ` [阵容:${bestHelper.lineupText}]` : ""} (已助战: ${helperUsageMap[bestHelper.id]}/4)`,
              type: "success",
            });
          } else {
            addLog({
        ...
        for (const car of carList) {
          ...
          try {
            ...
            const effectiveTickets = batchSettings.useGoldRefreshFallback
              ? 999
              : refreshTickets;
            ...
            if (
              shouldSendCar(
                ...
                batchSettings.smartDepartureMatchAll,
              )
            ) {
            ...
            while (shouldRefresh && !shouldStop.value) {
              ...
              if (refreshAttempts > maxRefreshAttempts) {
                ...
                addLog({
                  time: new Date().toLocaleTimeString(),
                  message: `${token.name} 车辆[${gradeLabel(car.color)}]达到最大刷新次数${maxRefreshAttempts}，停止刷新并发车`,
                  type: "warning",
                });
                await tokenStore.sendMessageWithPromise(
                  tokenId,
