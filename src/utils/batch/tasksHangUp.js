/**
 * 挂机、答题、签到类任务
 * 包含: claimHangUpRewards, batchAddHangUpTime, batchStudy, batchclubsign, batchClaimMailAttachment
 */
...
export function createTasksHangUp(deps) {
   ...
   * 一键领取邮件附件
   */
  const batchClaimMailAttachment = async () => {
    if (selectedTokens.value.length === 0)
      return;
    isRunning.value = true;
    shouldStop.value = false;
    ...
    const taskPromises = selectedTokens.value.map(async (tokenId) => {
      ...
      try {
        ...
        addLog({
          time: new Date().toLocaleTimeString(),
          message: `${token.name} 邮件附件领取失败: ${error.message || "未知错误"}`,
          type: "error",
        });
      } finally {
        tokenStore.closeWebSocketConnection(tokenId);
