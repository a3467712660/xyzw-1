...
export function createTasksItem(deps) {
  ...
  const sendWithRateLimitRetry = async (tokenId, cmd, body, timeout = 5000) => {
    const maxRetry = 2;
    for (let attempt = 0; attempt <= maxRetry; attempt++) {
      try {
        ...
        const waitMs = Math.max(delayConfig.action, 800) + attempt * 500;
        addLog({
          time: new Date().toLocaleTimeString(),
          message: `${tokens.value.find((t) => t.id === tokenId)?.name || tokenId} 操作过快，等待${Math.ceil(waitMs / 1000)}秒后重试`,
          type: "warning",
        });
