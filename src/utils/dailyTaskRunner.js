...
export class DailyTaskRunner {
  ...
  loadSettings(roleId) {
    try {
      ...
      const defaultSettings = {
        arenaFormation: 1,
        bossFormation: 1,
        legionBossFormation: 1,
        dailyBossFormation: 1,
        bossTimes: 2,
        claimBottle: true,
        recruitEnable: true,
        payRecruit: true,
  ...
  async run(tokenId, callbacks = {}, customSettings = null) {
    ...
    const baseSettings = this.loadSettings(tokenId) || {};
        ...
        taskList.push({
          ...
          execute: () =>
            this.switchToFormationIfNeeded(
              tokenId,
              settings.legionBossFormation || settings.bossFormation,
              "BOSS阵容",
            ),
        });
    ...
    if (settings.dailyBossEnable !== false) {
      ...
      taskList.push({
        ...
        execute: () =>
          this.switchToFormationIfNeeded(
            tokenId,
            settings.dailyBossFormation || settings.bossFormation,
            "BOSS阵容",
          ),
      });
      for (let i = 0; i < 3; i++) {
