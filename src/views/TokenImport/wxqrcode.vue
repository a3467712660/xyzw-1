  ...
  <div class="wx-qrcode-import">
    ...
    <div class="login-flow-info">
      ...
      <ol class="flow-steps">
        ...
        <li>
          {{ t("tokenImportWxQrcode.flow.step3Prefix") }}<strong color="red">{{ t("tokenImportWxQrcode.flow.step3Highlight") }}</strong>{{ t("tokenImportWxQrcode.flow.step3Suffix") }}
        </li>
      </ol>
    </div>
    ...
    <div class="form-actions">
      ...
      >
        ...
        {{ qrcodeUrl ? t("tokenImportWxQrcode.actions.refreshQrcode") : t("tokenImportWxQrcode.actions.getQrcode") }}
      </NButton>
    </div>
    ...
    <NForm
      ...
      :show-label="true"
    >
      <NFormItem :label="t('tokenImportWxQrcode.fields.nameTemplate')" :show-label="true">
        <NInput
          placeholder="{name}"
      ...
      <div class="mobile-only mobile-role-list">
        ...
        >
          <div class="mobile-role-head">
            ...
            <span class="mobile-role-power">{{ t("tokenImportWxQrcode.columns.power") }} {{ formatPower(role.power) }}</span>
          </div>
          <div class="mobile-role-meta">
            <span>{{ t("tokenImportWxQrcode.mobile.server") }} {{ getServerDisplay(role.serverId) }}</span>
            <span>{{ t("tokenImportWxQrcode.mobile.roleIndex") }} {{ getRoleIndex(role.serverId) }}</span>
            <span class="word-break-all">{{ t("tokenImportWxQrcode.mobile.roleId") }} {{ role.roleId }}</span>
          </div>
    ...
    <a-list>
      <a-list-item v-for="(role, index) in roleList" :key="index">
        <div class="role-item-row">
          <div>
            <strong>{{ t("tokenImportWxQrcode.roleLabels.name") }}</strong> {{ role.name || t("tokenImportWxQrcode.roleFallbacks.unnamed") }}<br>
            <strong>{{ t("tokenImportWxQrcode.roleLabels.token") }}</strong>
            <span class="word-break-all">{{ role.token }}</span><br>
            <strong>{{ t("tokenImportWxQrcode.roleLabels.server") }}</strong> {{ role.server || t("tokenImportWxQrcode.roleFallbacks.unspecified") }}<br>
            <strong>{{ t("tokenImportWxQrcode.roleLabels.roleIndex") }}</strong> {{ role.roleIndex }}
          </div>
...
const DEFAULT_XYZW_RUNTIME_ALLOWED_HOSTS = ["xyzw.xq5007.fun"];
const XYZW_RUNTIME_ALLOWED_HOSTS = [
  ...DEFAULT_XYZW_RUNTIME_ALLOWED_HOSTS,
  ...String(import.meta.env.VITE_XYZW_RUNTIME_ALLOWED_HOSTS || "")
    .split(",")
...
const ensureRuntimeHostAllowed = () => {
  ...
  if (!host)
    throw new Error(t("tokenImportWxQrcode.errors.runtimeHostNotAllowed", { host: "unknown" }));
  if (isLoopbackHost(host))
    return;
  if (XYZW_RUNTIME_ALLOWED_HOSTS.some((pattern) => matchesAllowedHost(host, pattern)))
    return;
  throw new Error(t("tokenImportWxQrcode.errors.runtimeHostNotAllowed", { host }));
};

const loadRuntimeScript = (src: string) =>
  new Promise<void>((resolve, reject) => {
    const existing = document.querySelector(`script[${XYZW_RUNTIME_SCRIPT_ATTR}="${src}"]`) as HTMLScriptElement | null;
    if (existing) {
...
const addSelectedRole = async (roleInfo: any) => {
  ...
  try {
    ...
    const roleName = roleInfo.name || t("tokenImportWxQrcode.messages.roleFallback", { roleId: roleInfo.roleId });
    ...
    if (exists) {
      message.warning(t("tokenImportWxQrcode.messages.roleAlreadyQueued", { name: finalName }));
      return;
    }
...
const tryGetWeixinQR = async () => {
  try {
    ...
    const response = await new Promise((resolve, reject) => {
      ...
      xhr.onerror = () => reject(new Error(t("tokenImportWxQrcode.errors.networkError")));
      xhr.ontimeout = () => reject(new Error(t("tokenImportWxQrcode.errors.requestTimeout")));
      xhr.send();
    });

    if (response.status !== 200) {
      throw new Error(t("tokenImportWxQrcode.errors.httpStatus", { status: response.status }));
    }
...
const checkScanStatus = async () => {
  try {
    ...
    // 使用微信官方推荐的扫码状态轮询路径
    const url
      = `${WECHAT_PROXY_BASE}/qrstatus?uuid=${
        qrcodeUUID.value
      }&f=url&_=${
        Date.now()}`;
...
const getEncryptedData = async (code) => {
  ...
  const res = await new Promise((resolve, reject) => {
    ...
    xhr.onload = () => resolve(xhr);
    xhr.onerror = () => reject(new Error(t("tokenImportWxQrcode.errors.loginFailed")));
    xhr.ontimeout = () => reject(new Error(t("tokenImportWxQrcode.errors.loginTimeout")));
    xhr.send(encoded);
  });

  if (res.status !== 200) {
    throw new Error(t("tokenImportWxQrcode.errors.httpStatus", { status: res.status }));
  }

  const json = JSON.parse(res.responseText);
  if (json.meta?.errCode !== 0) {
    throw new Error(t("tokenImportWxQrcode.errors.loginFailedWithReason", { error: json.meta?.errMsg }));
  }
