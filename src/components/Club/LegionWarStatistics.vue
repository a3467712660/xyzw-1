...
const exportImage = async () => {
  if (!isAccessible.value) {
    message.warning("当前不在盐场开放时间");
    return;
  }
  const element = document.querySelector(
    ".legion-war-statistics-card .table-content",
  );
  if (!element) {
...
const legionData = computed(() => {
  ...
  return Object.values(validData.value.legionInfo)
    .map((item) => {
      ...
      const realRedCount =
        detail?.quenchNum !== undefined ? detail.quenchNum : item.redCount;
      // 优先使用详情中的战力数据 (power)
...
const individualData = computed(() => {
  ...
  return Object.values(validData.value.memberInfo)
    ...
    .map((item, index) => ({
      ...
      key: index,
      kd:
        item.die > 0 ? (item.kill / item.die).toFixed(2) : item.kill.toFixed(2),
    }))
...
onMounted(() => {
  if (!isAccessible.value) {
    // 非开放时间：确保不建立连接
    legionWarStore.disconnect(true);
    return;
  }
  // 组件加载时尝试连接
  try {
