  ...
  <div>
    ...
    <div v-if="inline" class="inline-wrapper">
      ...
      <div class="weird-tower-content">
        ...
        >
          <div v-if="isExporting" class="export-banner">
            <div class="export-title">俱乐部怪异塔排行</div>
            <div class="export-club-name">俱乐部：{{ exportClubName }}</div>
            <div class="export-subtitle">
              导出时间 {{ exportTimeText }} · 共
              {{ memberScores.length }} 名成员
            </div>
          </div>
          <NDataTable
            striped
...
const exportClubName = computed(() => {
  return tokenStore.gameData?.legionInfo?.info?.name || "未知俱乐部";
});
const exportTimeText = computed(() => {
  const now = new Date();
...
.records-list.is-exporting-image {
  padding: 16px;
  border-radius: 16px;
...
.records-list.is-exporting-image :deep(.n-data-table) {
  border-radius: 12px;
  overflow: hidden;
...
.records-list.is-exporting-image :deep(.n-data-table-th) {
  background: linear-gradient(180deg, #f2f7ff 0%, #eaf2ff 100%);
  color: #294676;
  font-weight: 700;
}

.records-list.is-exporting-image :deep(.n-data-table-td) {
