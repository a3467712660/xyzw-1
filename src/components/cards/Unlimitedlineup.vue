...
import {
  ...
  getPearlSkillId,
  getLineupTeamHeroes as getTeamHeroes,
  isArtifactAssignedToTargetHero,
  reviewLineupEquipment,
} from "./lineup/lineupMatching";
import {
  color,
  FishMap,
  getTechType,
...
const applyLineup = async (lineup, options = {}) => {
  ...
  try {
    ...
    const getEquipmentReview = (heroesData = {}) =>
      reviewLineupEquipment(targetHeroes, heroesData, resolveHeroName);
    ...
    const exchangeHeroesLosslessly = async (
      ...
      const emptySlots = getLineupSlotCandidates(teamHeroes, targetHeroes).filter(
