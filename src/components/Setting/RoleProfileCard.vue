  ...
  >
    <div class="role-profile-content">
      ...
      <div class="avatar-container">
        <img
          class="role-avatar"
          :alt="roleInfo.name || t('roleProfileCard.common.role')"
          :src="roleAvatar"
      ...
      <div class="role-info-section">
        <div class="role-name">{{ roleInfo.name || t("roleProfileCard.common.unknownRole") }}</div>
        <div class="role-stats">
          <span class="level-text">Lv.{{ roleInfo.level || 1 }}</span>
          <span class="power-value">{{ t("roleProfileCard.labels.power", { value: formatPower(roleInfo.power) }) }}</span>
        </div>
      </div>
      ...
      <div class="progress-section">
        ...
        <div class="progress-text">
          ...
          <span v-else>{{ t("roleProfileCard.common.max") }}</span>
        </div>
      </div>
    </div>
...
const powerRanks = computed(() => [
  {
    min: 0,
    max: 1000000,
    title: t("roleProfileCard.ranks.beginner.title"),
    description: t("roleProfileCard.ranks.beginner.description"),
    icon: "🌱",
    class: "rank-beginner",
    color: "#6b7280",
  },
  {
    min: 1000000,
    max: 10000000,
    title: t("roleProfileCard.ranks.known.title"),
    description: t("roleProfileCard.ranks.known.description"),
    icon: "⚔️",
    class: "rank-known",
    color: "#10b981",
  },
  {
    min: 10000000,
    max: 100000000,
    title: t("roleProfileCard.ranks.veteran.title"),
    description: t("roleProfileCard.ranks.veteran.description"),
    icon: "🗡️",
    class: "rank-veteran",
    color: "#3b82f6",
  },
  {
    min: 100000000,
    max: 500000000,
    title: t("roleProfileCard.ranks.master.title"),
    description: t("roleProfileCard.ranks.master.description"),
    icon: "🏹",
    class: "rank-master",
    color: "#8b5cf6",
  },
  {
    min: 500000000,
    max: 2000000000,
    title: t("roleProfileCard.ranks.hero.title"),
    description: t("roleProfileCard.ranks.hero.description"),
    icon: "⚡",
    class: "rank-hero",
    color: "#f59e0b",
  },
  {
    min: 2000000000,
    max: 4000000000,
    title: t("roleProfileCard.ranks.overlord.title"),
    description: t("roleProfileCard.ranks.overlord.description"),
    icon: "👑",
    class: "rank-overlord",
    color: "#ef4444",
  },
  {
    min: 4000000000,
    max: 6000000000,
    title: t("roleProfileCard.ranks.supreme.title"),
    description: t("roleProfileCard.ranks.supreme.description"),
    icon: "🔱",
    class: "rank-supreme",
    color: "#ec4899",
  },
  {
    min: 6000000000,
    max: 9000000000,
    title: t("roleProfileCard.ranks.emperor.title"),
    description: t("roleProfileCard.ranks.emperor.description"),
    icon: "⚜️",
    class: "rank-emperor",
    color: "#dc2626",
  },
  {
    min: 9000000000,
    max: 15000000000,
    title: t("roleProfileCard.ranks.legend.title"),
    description: t("roleProfileCard.ranks.legend.description"),
    icon: "💎",
    class: "rank-legend",
    color: "#7c3aed",
  },
  {
    min: 15000000000,
    max: Infinity,
    title: t("roleProfileCard.ranks.infinite.title"),
    description: t("roleProfileCard.ranks.infinite.description"),
    icon: "🌟",
    class: "rank-infinite",
    color: "#fbbf24",
  },
...
const rankInfo = computed(() => {
  ...
  const rank = powerRanks.value.find((rank) => power >= rank.min && power < rank.max);
  return rank || powerRanks.value[0];
});

// 计算下一个段位门槛
const nextRankThreshold = computed(() => {
  const currentRankIndex = powerRanks.value.findIndex(
    (rank) => rank === rankInfo.value,
  );
  if (currentRankIndex >= 0 && currentRankIndex < powerRanks.value.length - 1) {
    return powerRanks.value[currentRankIndex + 1].min;
  }
  return null;
});
...
const formatPower = (power) => {
  ...
  if (power >= yi) {
    ...
    return t("roleProfileCard.units.yi", { value });
  } else if (power >= wan) {
    const value = (power / wan).toFixed(1);
    return t("roleProfileCard.units.wan", { value });
  } else {
