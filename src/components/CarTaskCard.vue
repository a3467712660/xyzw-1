    ...
    <div class="card-content">
      ...
      <div class="car-grid">
        <div
          ...
          :key="car.id"
          class="car-item car-item-width-reset"
        >
          ...
          <div v-if="car.rewards && car.rewards.length > 0" class="car-rewards">
            ...
            >
              ...
              <span
                ...
                class="big-prize"
              >
                {{ t("carTaskCard.bigPrizeTag") }}
              </span>
            </text>
            <div v-if="isBigPrize(car.rewards)" class="big-prize-badge">
              {{ t("carTaskCard.bigPrizeBadge") }}
            </div>
          </div>
          <div class="car-actions-inline">
            <button class="query-btn" @click="refreshCar(car.id)">
              {{ t("carTaskCard.actions.refresh") }}
            </button>
            <button class="query-btn" @click="claimCar(car.id)">
              {{ t("carTaskCard.actions.claim") }}
            </button>
            <button class="query-btn" @click="sendCar(car.id)">
              {{ t("carTaskCard.actions.send") }}
            </button>
          </div>
        </div>
      </div>
      <div class="car-actions-group">
        <button class="query-btn" @click="smartSendCar">
          {{ t("carTaskCard.actions.smartSend") }}
        </button>
        <button class="query-btn" @click="claimAllCars">
          {{ t("carTaskCard.actions.claimAll") }}
        </button>
      </div>
    </div>
  </div>
...
const getColorName = (color) => {
  const colorMap = {
    1: t("carTaskCard.colors.common"),
    2: t("carTaskCard.colors.rare"),
    3: t("carTaskCard.colors.epic"),
    4: t("carTaskCard.colors.legendary"),
    5: t("carTaskCard.colors.mythic"),
  };
  return colorMap[color] || t("carTaskCard.colors.unknown", { color });
};
...
const parseCarRewards = (rewards) => {
  ...
  for (const reward of rewards) {
    ...
    if (rewardType === 1) {
      // 金币
      rewardInfo.push(t("carTaskCard.rewardTypes.coin", { value }));
    } else if (rewardType === 2) {
      // 金砖
      rewardInfo.push(
        t("carTaskCard.rewardTypes.ingot", { value: value.toLocaleString() }),
      );
    } else if (rewardType === 3) {
      // 物品
      const itemName = getItemName(itemId);
      rewardInfo.push(t("carTaskCard.rewardTypes.item", { item: itemName, value }));
    } else {
      rewardInfo.push(
        t("carTaskCard.rewardTypes.other", {
          type: rewardType,
          itemId,
          value,
        }),
...
const getCarStatusText = (carData) => {
  ...
  if (sendAt > 0 && claimAt === 0 && rewards.length > 0) {
    return t("carTaskCard.status.claimable");
  } else if (sendAt > 0) {
    // 已发车
    return t("carTaskCard.status.sent");
  } else {
    // 未发车
    return t("carTaskCard.status.unsent");
  }
};
    ...
    try {
      ...
      if (result && result.roleCar && result.roleCar.carDataMap) {
        ...
        for (const carId in carDataMap) {
          if (Object.prototype.hasOwnProperty.call(carDataMap, carId)) {
    ...
    try {
      ...
      for (const car of sortedCarList.value) {
        if (car.status === "unsent") {
          ...
          // 获取当前车辆的刷新次数
          let refreshCount = car.refreshCount || 0;
